...
/**
 * The terrain view, and what stands in for it when there is no GPU. `next/dynamic` with
 * `ssr: false` is only allowed inside a Client Component, which is why this wrapper
 * exists: it keeps three.js out of the server render and out of the initial bundle.
 */
...
/**
 * What the viewer reads instead of the mountain. Both reasons the scene can be missing say
 * the same thing: the numbers come from the elevation model, not from the renderer.
 */
...
/**
 * Keeps a failed terrain load inside the canvas's own frame. Without this the rejection
 * reaches Next's root error boundary and replaces the whole document, including the facts
 * and the disclaimer, which are the page whenever the 3D view cannot be (SPEC §9).
 */
  ...
  return (
    <div className={`relative ${FRAME}`}>
      ...
      {/* Aerial perspective: the sky the massif fades into, so distance reads as
          distance. The scene's fog matches the horizon end so the mesh has no seam. */}
      <div
        aria-hidden="true"
